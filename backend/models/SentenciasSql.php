...
//        die();
    ...
    public function fechasDisponiblesSemana($desde, $hasta, $clase, $bloque, $semanaId) {
        $con = Yii::$app->db;
        $query = "SELECT date_trunc('day', dd):: date as fecha
                        ,extract(dow from date_trunc('day', dd):: date) as numero_dia
                        ,(select nombre from scholaris_horariov2_dia where numero = extract(dow from date_trunc('day', dd):: date)) as dia
...
group by cri.id 
		,cri.criterio order by cri.criterio ;";
//        print_r($query);
//        die();
        $res = $con->createCommand($query)->queryAll();
        return $res;
...
//        die();
    ...
    public function cuadroEstadistico($periodo, $claseId, $bloqueId) {
        $con = Yii::$app->db;
        $query = "select 	t.id
		,t.abreviatura
		,t.descripcion
		...
		,(
			...
			from scholaris_resumen_parciales res
					inner join op_student_inscription ins on ins.student_id = res.alumno_id
					inner join scholaris_op_period_periodo_scholaris sop on sop.op_id = ins.period_id
					inner join scholaris_periodo sp on sp.id = sop.scholaris_id
			where res.clase_id = $claseId
					and res.bloque_id = $bloqueId 
					and res.calificacion 
					between t.rango_minimo 
					and t.rango_maximo 
					and sp.codigo = '$periodo'
					and ins.inscription_state = 'M'
		 )
from	scholaris_tabla_escalas_homologacion t
where	t.scholaris_periodo = '$periodo'
...
//        die();
    ...
    public function promedioParcialClase($claseId, $bloqueId, $periodo) {
        $con = Yii::$app->db;
        $query = "select trunc(avg(r.calificacion),2) as promedio 
                    from 	scholaris_resumen_parciales r
                                    inner join op_student_inscription i on i.student_id = r.alumno_id
                                    inner join scholaris_op_period_periodo_scholaris sop on sop.op_id = i.period_id
                                    inner join scholaris_periodo sp on sp.id = sop.scholaris_id
                    where 	r.clase_id = $claseId 
                                    and r.bloque_id = $bloqueId
                                    and sp.codigo = '$periodo'
                                    and i.inscription_state = 'M';";
//        print_r($query);
//        die();
